...
public:
    ...
    Note ( const std::string& ti, const std::string& te): title(ti), text(te){};  //costruttore, passo il titolo e il testo. La nota non è bloccata di default.
    ...
    bool setTitle(const std::string &title); //passaggio per riferimento: opera direttamente sullì'argomento passato. Non fa la copia dei dati (possibilmente troppo lunghi)
